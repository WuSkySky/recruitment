...
const playerTeam = ref<"red" | "blue">("red");
...
const canEnter = computed(
  () => connection.value === "connected" && videoReady.value && !statusStalled.value && !videoStalled.value,
);
...
function connectWebSocket(): void {
  ...
  socket.addEventListener("message", async (event) => {
    ...
    if (message.type === "ready") {
      ...
      playerTeam.value = message.player_team;
...
onMounted(() => {
  ...
  sendTimer = window.setInterval(
    () => sendInput(pointerLocked.value && connection.value === "connected"),
    1000 / 60,
  );
  ...
  >
    ...
    <header class="scoreboard">
      ...
      <section class="match-clock">
        <span>ROUND {{ status?.match.available ? "01" : "--" }}</span>
