...
: node_(node), gimbal_vel_actuator_(gimbal_vel_actuator), gimbal_pos_sensor_(gimbal_pos_sensor)
{
  ...
  gimbal_pos_sensor->add_callback(
    [this](const recruitment_sim_interfaces::msg::Gimbal & data, const rclcpp::Time & /*stamp*/) {
      cur_yaw_ = data.yaw;
      cur_pitch_ = data.pitch;
    });
  // ros pub and sub
  using namespace std::placeholders;
  auto sim_gimbal_cmd_topic = "robot_base/gimbal_cmd";
  auto sim_gimbal_state_topic = "robot_base/gimbal_state";
  auto gimbal_joint_cmd_topic = "cmd_gimbal_joint";
  sim_gimbal_state_pub_ = node_->create_publisher<recruitment_sim_interfaces::msg::Gimbal>(
    sim_gimbal_state_topic, 10);
  sim_gimbal_cmd_sub_ = node_->create_subscription<recruitment_sim_interfaces::msg::GimbalCmd>(
    sim_gimbal_cmd_topic, 10, std::bind(&GimbalController::gimbal_cb, this, _1));
...
{
  recruitment_sim_interfaces::msg::Gimbal cmd;
  // pid for pitch
...
{
  recruitment_sim_interfaces::msg::Gimbal gimbal_pos;
  gimbal_pos.pitch = cur_pitch_;
  gimbal_pos.yaw = cur_yaw_;
  sim_gimbal_state_pub_->publish(gimbal_pos);
}
...
void GimbalController::gimbal_cb(const recruitment_sim_interfaces::msg::GimbalCmd::SharedPtr msg)
{
  // for pitch
